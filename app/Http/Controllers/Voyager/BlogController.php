...
class BlogController  extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $orderBy = $request->get('order_by', $dataType->order_column); // sắp xếp theo
        ...
        if(Auth::user()->can('read_all', app($dataType->model_name))){
            $dataTypeContent = Blog::orderBy($dataType->order_column, $dataType->order_direction)->get();
        }
        else
        {
            $dataTypeContent = Blog::where('author_id', '=', Auth::user()->id)->orderBy($dataType->order_column, $dataType->order_direction)->get();
        }
        // Actions
        $actions = [];
