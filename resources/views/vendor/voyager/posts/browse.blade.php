                                                ...
                                                @foreach ($actions as $action)
                                                    @if (!method_exists($action, 'massAction'))
                                                        @if ($data)
                                                            ...
                                                            @can($action->getPolicy(), $data)
                                                                @if ($action->shouldActionDisplayOnRow($data))
                                                                    @if ($action->getIcon() == 'voyager-eye')
                                                                        {{-- đường dẫn đến bài viết --}}
                                                                        <a href="{{ asset('post').'/'.TCG\Voyager\Models\Category::where('id',$data->category_id)->first()->slug.'/'. $data->slug}}"
                                                        ...
                                                        @elseif (method_exists($action, 'massAction'))
                                                            <form method="post"
                                                                action="{{ route('voyager.' . $dataType->slug . '.action') }}"
                                                                style="display:inline">
                                                                {{ csrf_field() }}
                                                                <button type="submit" {!! $action->convertAttributesToHtml() !!}><i
                                                                        class="{{ $action->getIcon() }}"></i>
